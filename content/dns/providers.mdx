...
Root domains (`acme.com` rather than `www.acme.com`) depend on what the
provider can hold at the zone apex — see [Root domains](/docs/dns/root-domains).
For records only you know (SES DKIM, a verification `TXT`), see
[Bring your own records](/docs/dns/bring-your-own-records).
...
### What actually works today, per provider

Measured against production, not the adapter list. "Automatic" means we can
write the records for you; "manual" means you add the records we show.

| Provider | Root domain | How it connects today | Notes |
| --- | --- | --- | --- |
| Cloudflare | ✅ ALIAS-style (flattened CNAME) | **Sign in with Cloudflare** (OAuth, one click) or an API token | The best-supported path. |
| Vercel · Netlify · DNSimple · WordPress.com | ✅ | **Sign in with the provider** (OAuth) | Registered and live. |
| GoDaddy | ✅ via **A records** at `@` | **Manual** for most accounts | GoDaddy issues API access only to 10+ domain / paid accounts; its one-click (Domain Connect) rail needs GoDaddy-side onboarding that isn't complete. |
| Namecheap · Name.com | ✅ via **A records** at `@` | API token, or manual | No ALIAS type at the root — use the A records the console shows. |
| Route 53 | ✅ ALIAS | Access key + secret (+ session token for temporary credentials) | IAM user with `ChangeResourceRecordSets` on the zone. |
| OVH · Google Cloud DNS · Azure DNS · DigitalOcean | ✅ / varies | API credentials, or manual | DigitalOcean OAuth is not registered yet. |
| NameSilo · Glauca Digital · Domain Chief | ✅ | **One-click setup** (Domain Connect) | These providers serve our templates. |
| Wix · Squarespace · Hover · anything else | depends on the panel | Manual | Add the records we show. |

The console detects your provider from the domain's nameservers and shows
only the paths that exist for it.
