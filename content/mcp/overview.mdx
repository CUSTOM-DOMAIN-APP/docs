...
| `reapply-connection` | `{ connectionId }` | Re-apply a managed connection: the control-plane recomputes its DNS from stored config and re-pushes it through the stored grant. Never supply records. Fails (`409`) if the connection isn't managed or has no grant — which is every connection on the hosted service today, see [Managed connections](/docs/connect-flow/managed-connections). |
| `disconnect-domain` | `{ connectionId }` | Disconnect a managed connection: the control-plane reverts its DNS through the stored grant, then deletes the grant and connection. Never supply records. Same availability note as `reapply-connection`. |
...
For these **integrator** credentials the MCP server does not validate tokens
cryptographically — it forwards the caller's credential to the control-plane,
which is the authority and rejects bad credentials on every wrapped call.

**Delegated agent tokens are the exception.** When a human owner grants an AI
agent access, the agent presents a short-lived **ES256** token that the MCP
verifies **locally** against the control-plane JWKS (no per-request callback) and
enforces the tool's required scope on — a read-only grant can never reach a
connect or purchase tool. See [Agent access](/docs/agents/overview) for the
human-first consent flow.
