...
> infra/config.alloy is pre-wired to Customdomain's own Grafana Cloud tenant (hardcoded basic-auth users + prod-us-east endpoints) and infra/observability.env ships a real token; a self-hoster who runs docker-compose.alloy.yml as-is forwards THEIR logs/metrics/traces into Customdomain's tenant. The Alloy path is NOT a simple env-var repoint — config.alloy must be replaced/re-pointed at the self-hoster's own Grafana stack (and observability.env supplied with their own token). The default no-overlay zero-egress claim for the base stack remains true.
