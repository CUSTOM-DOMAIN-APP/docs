...
| `failed` | A `propagating` connection's records never appeared within the 24-hour window (or, for a manual connection, within 72 hours of `pending`). |
...
simply waits for the records to appear — giving the customer up to **72 hours**
(long enough for a human to get to it, unlike the 24-hour automatic-rail window)
before giving up and marking the connection `failed` too.

A `failed` connection carries `error_code` / `error_message` naming why:
`propagation_timeout` (a `propagating` connection's records never resolved) or
`setup_incomplete` (a manual connection's records were never added at all).
Both are cleared automatically if the connection later re-verifies.
