...
> **Availability on the hosted service:** managed mode needs an async one-click
> grant from the DNS provider, which requires provider onboarding
> (`OAUTH_DC_<PROVIDER>_CLIENT_ID`). No provider has issued one to
> customdomain.ai yet, so on the hosted service every connection is use-once,
> `managed` is always `false`, and `:reapply` / the MCP `reapply-connection` and
> `disconnect-domain` tools return `409`. Drift is reported, not healed.
> Self-hosters who complete onboarding get the behaviour below.
...
The `200` body is a re-apply **receipt**, not a connection object: it carries
